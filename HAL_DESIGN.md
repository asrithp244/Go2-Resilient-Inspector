...
This document defines the exact boundary between the **mock hardware abstraction layer** (`go2_hal_node`) used in simulation and the **real Unitree Go2 SDK** it replaces. Any engineer reading this should be able to understand what would need to change to deploy this system on physical hardware.
...
| Foot contact: constant 120N per leg (simulated) | Parses `LowState.footForce[]` (4 × uint16 contact force sensors) |
| Velocity commands: no forwarding needed (Gazebo direct) | Calls `SportModeSDK.Move(vx, vy, vyaw)` via UDP control channel |

The topic names, message types, QoS profiles, and frame IDs published by `go2_hal_node` are **identical in mock and real modes**. All downstream nodes (EKF, Nav2, BT) are unaware of which mode is active.
...
Both mock and real modes publish this tf2 tree:

```
map
 └── odom                    (published by: robot_localization EKF)
      └── base_link           (published by: go2_hal_node via odom->base_link transform)
...
| `/go2/imu` | BEST_EFFORT, depth=5 | 400 Hz — same |
| `/go2/odom` | RELIABLE, depth=10 | EKF depends on every odom message for integration |
...
- All downstream node code (EKF, health_monitor, mission_bt_node, report_gen)
- Topic names and message types
- QoS profiles
- tf2 frame names
- Nav2 costmap and planner configuration
...
**Estimated engineering effort to deploy on real hardware: 1–2 days** (network setup, UDP timing tuning, safety testing).
...
- The mock is honest: `HAL_DESIGN.md` documents what the mock does NOT test

This approach is directly adapted from real field robotics practice: the HAL boundary is the same pattern used in Boston Dynamics Spot SDK, Clearpath Husky HAL, and NASA's JPL Robot Operating System (ROS-based) projects.
